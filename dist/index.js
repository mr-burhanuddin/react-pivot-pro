import { useState, useRef, useMemo, useCallback, useEffect, useReducer, useLayoutEffect } from 'react';
import { useStore } from 'zustand';
import { createStore } from 'zustand/vanilla';
import { Virtualizer, elementScroll, observeElementOffset, observeElementRect, observeWindowRect, observeWindowOffset, windowScroll } from '@tanstack/virtual-core';
...
function useVirtualRows(options) {
  ...
  const resolvedOptions = useMemo(() => {
    ...
    const observeRect = customObserveElementRect ? customObserveElementRect : isWindowMode ? observeWindowRect : observeElementRect;
    const observeOffset = customObserveElementOffset ? customObserveElementOffset : isWindowMode ? observeWindowOffset : observeElementOffset;
    const scrollToFn = customScrollToFn ? customScrollToFn : isWindowMode ? windowScroll : elementScroll;
    const nextOptions = {
      count,
      getScrollElement,
      estimateSize,
...
export { DEFAULT_MANIFESTS, aggregationFns, copyToClipboard, createColumnOrderingPlugin, createColumnPinningPlugin, createColumnVisibilityPlugin, createDefaultTableState, createDndColumnPlugin, createDndRowPlugin, createFilteringPlugin, createGroupingPlugin, createPivotEngineResult, createPivotPlugin, createPivotTableStore, createPluginRegistry, createSortingPlugin, exportCSV, fullscreen, resolveAggregationFn, serializeCSV, useDndColumn, useDndRow, useFiltering, useGrouping, usePivot, usePivotTable, useSorting, useVirtualColumns, useVirtualRows, withColumnOrdering, withColumnPinning, withColumnVisibility, withDndColumn, withDndRow, withFiltering, withGrouping, withPivot, withSorting };
