...
const PRODUCT_CATEGORY_MAP: Record<string, string> = {
  ...
  "Tool Pro": "Software",
  "Tool Lite": "Software",
  "Suite Enterprise": "Services",
  "Suite Basic": "Services",
};
const SALES_REPS = [
  "Alice Chen",
  "Bob Martinez",
...
const REP_PERFORMANCE: Record<
  string,
  { marginBias: number; volumeBias: number }
> = {
...
export function generateMockData(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const revenue =
      Math.round(units * unitPrice * (1 - discountPct / 100) * 100) / 100;
    const cost = Math.round(units * baseCost * productMultiplier * 100) / 100;
    const margin =
      Math.round(((revenue - cost) / revenue) * 1000 + perf.marginBias * 10) /
      10;
