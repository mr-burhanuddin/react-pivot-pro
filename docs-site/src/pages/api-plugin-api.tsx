...
export default function ApiPluginApi() {
  return (
    <DocPage title="Plugin API" subtitle="Core plugin interface and lifecycle hooks for extending react-pivot-pro">
      <h2>PivotTablePlugin Interface</h2>
      <p>Every plugin implements the <code>PivotTablePlugin</code> interface:</p>
      <pre><code>{`interface PivotTablePlugin<TData extends RowData, TState extends TableState = TableState> {
  name: string;
  ...
  onStateChange?: (
    ...
    context: PivotTablePluginContext<TData, TState>,
      ...
      <pre><code>{`interface PivotTablePluginContext<TData extends RowData, TState extends TableState = TableState> {
  columns: Column<TData>[];
  data: TData[];
  state: TState;
  ...
  getColumnById: (columnId: string) => Column<TData> | undefined;
      ...
      <p>Called once when the plugin is first registered. Merges plugin-specific state into the table state.</p>
      <pre><code>{`getInitialState: (state) => ({
  ...state,
  sorting: state.sorting ?? [],
})`}</code></pre>

      <h3>2. transformRows</h3>
      <p>Called on every render cycle. Receives the current row array and returns a transformed version. Plugins are applied in registration order.</p>
      <pre><code>{`transformRows: (rows, context) => {
  ...
  return [...rows].sort((a, b) => {
    // sorting logic
      ...
      <p>Called on every render cycle. Receives and returns a transformed column array. Used for column visibility, ordering, and pinning.</p>
      <pre><code>{`transformColumns: (columns, context) => {
  const visibility = context.state.columnVisibility ?? {};
  return columns.filter(col => visibility[col.id] !== false);
}`}</code></pre>

      <h3>4. onStateChange</h3>
      <p>Called after state changes. Use for validation, normalization, or side effects.</p>
      <pre><code>{`onStateChange: (state, previousState, context) => {
  ...
  if (filtered.length !== state.sorting.length) {
    context.setState(prev => ({ ...prev, sorting: filtered }));
      ...
      <pre><code>{`export function createSortingPlugin<TData extends RowData, TState extends SortingTableState>(
  ...
  return {
    ...
    transformRows: (rows, context) => { /* ... */ },
      ...
      <pre><code>{`export function withSorting<TData extends RowData, TState extends SortingTableState>(
  ...
  return Object.assign(table, {
    sorting: createSortingApi(table, options),
      ...
      <h3>Hook Pattern</h3>
      <pre><code>{`export function useSorting<TData extends RowData, TState extends SortingTableState>(
  table: PivotTableInstance<TData, TState>,
  ...
  return createSortingApi(table);
      ...
      <h2>Caching</h2>
      <p>Plugins should implement caching to avoid unnecessary recomputation. The engine caches plugin output keyed by input reference equality:</p>
...
transformRows: (rows, context) => {
  ...
  return result;
      ...
      <pre><code>{`// Register a plugin after mount
...
const all = table.getAllPlugins();`}</code></pre>
